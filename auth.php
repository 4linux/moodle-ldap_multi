...
/**
 * Authentication Plugin: LDAP Authentication | FORK from Original LDAP Plugin
 * Authentication using LDAP (Lightweight Directory Access Protocol).
 *
 * @package auth_ldap
 * @author Martin Dougiamas
 * @author Iñaki Arenaza
 * @author Denis Perciliano Ribeiro {@link http://denisribeiro.com.br}
 ...
 */
class auth_plugin_ldap_multi extends auth_plugin_base
{
    ...
    function init_plugin($authtype)
    {
        ...
        $userType = isset($this->config->user_type) ? json_decode($this->config->user_type) : [
            'default'
        ];
        ...
        $this->config->user_type_name = $ldap_usertypes[$userType[0]];
        ...
        foreach ($default as $key => $value) {
            ...
            if (! isset($this->config->{$key}) or $this->config->{$key} == '') {
                $this->config->{$key} = $value[$userType[0]];
            }
        }
        
        $objectClass = json_decode($this->config->objectclass);
        
        // Hack prefix to objectclass
        $this->config->objectclass = ldap_normalise_objectclass($objectClass[0]);
    }
    ...
    {
        $this->authtype = 'ldap_multi';
        $this->roleauth = 'auth_ldap_multi';
     ...
     */
    public function auth_plugin_ldap_multi()
    {
     ...
     *            The username (without system magic quotes)
     * @param string $password
     *            The password (without system magic quotes)
     *            
    ...
    function user_login($username, $password)
    {
        ...
        $numberOfServers = count(json_decode($this->config->host_url));
        $ldapconnections = [];
        
        for ($x = 0; $x < $numberOfServers; $x ++) {
            ...
            $optsDeref[$x] = isset($optsDeref[$x]) ? $optsDeref[$x] : 0;
            $startsTls = json_decode($this->config->start_tls);
            $ldapencoding = json_decode($this->config->ldapencoding);
            $expirations = json_decode($this->config->expiration);
            $stdchangepasswords = json_decode($this->config->stdchangepassword);
            $changepasswordurls = json_decode($this->config->changepasswordurl);
            $ntlmsso_enableds = json_decode($this->config->ntlmsso_enabled);
            $contexts = json_decode($this->config->contexts);
            $create_context = json_decode($this->config->create_context);
            $objectclass = json_decode($this->config->objectclass);
            $objectclass[$x] = is_string($this->config->objectclass) ? $this->config->objectclass : $objectclass[$x];
            $user_attribute = json_decode($this->config->user_attribute);
            $search_sub = json_decode($this->config->search_sub);
            
            $extusername = core_text::convert($username, 'utf-8', $ldapencoding[$x]);
            $extpassword = core_text::convert($password, 'utf-8', $ldapencoding[$x]);           
            // Before we connect to LDAP, check if this is an AD SSO login
            // if we succeed in this block, we'll return success early.
            //
                        
            $ldapconnection = $this->ldap_connect($hosts[$x], $verstions[$x], $types[$x], $bindsDn[$x], $bindsPw[$x], $optsDeref[$x], $startsTls[$x]);
            $ldap_user_dn = $this->ldap_find_userdn($ldapconnection, $extusername, $contexts[$x], $create_context[$x], $objectclass[$x], $user_attribute[$x], $search_sub[$x]);
            ...
            if (! $ldap_login && ($types[$x] == 'ad') && $this->can_change_password($stdchangepasswords[$x], $changepasswordurls[$x]) && (! empty($expirations[$x]) and ($expirations[$x] == 1))) {
    ...
    function get_userinfo($username)
    {
        ...
        for ($x = 0; $x < $numberOfServers; $x ++) {
            ...
            if (! ($user_dn = $this->ldap_find_userdn($ldapconnection, $extusername, $contexts[$x], $create_context[$x], $objectclass[$x], $user_attribute[$x], $search_sub[$x]))) {
                $this->ldap_close();
                continue;
            }
            ...
            foreach ($attrmap as $key => $values) {
                ...
                foreach ($values as $value) {
                    ...
                    if (is_array($entry[$value])) {
                        $newval = core_text::convert($entry[$value][0], $ldapencoding[$x], 'utf-8');
                    } else {
                        $newval = core_text::convert($entry[$value], $ldapencoding[$x], 'utf-8');
                    }
            ...
            if(! isset($result['email']) || ! $result['email']){
                $email  = trim($result['firstname'], ' ') . '_' . microtime();
                $result['email'] = strtolower(str_replace(' ', '', "$email@noemail.com.br"));
            }
    ...
    function user_exists($username)
    {
        ...
        for ($x = 0; $x < $numberOfServers; $x ++) {
            ...
            $users = $this->ldap_get_userlist('(' . $user_attribute[$x] . '=' . ldap_filter_addslashes($extusername) . ')');
            return count($users);
        }
    }
    ...
    function user_create($userobject, $plainpass)
    {
        ...
        for ($x = 0; $x < $numberOfServers; $x ++) {
            ...
            $bindsDn = json_decode($this->config->bind_dn);
            $bindsPw = json_decode($this->config->bind_pw);
            $passtype = json_decode($this->config->passtype);
            $optsDeref = json_decode($this->config->opt_deref);
            $optsDeref[$x] = isset($optsDeref[$x]) ? $optsDeref[$x] : 0;
            $startsTls = json_decode($this->config->start_tls);
            $ldapencoding = json_decode($this->config->ldapencoding);
            ...
            $extusername = core_text::convert($userobject->username, 'utf-8', $ldapencoding[$x]);
            $extpassword = core_text::convert($plainpass, 'utf-8', $ldapencoding[$x]);
     ...
     *            print notice with link and terminate
    ...
    function sync_users($do_updates = true)
    {
        ...
        $table->add_index('username', XMLDB_INDEX_UNIQUE, array(
            'mnethostid',
            'username'
        ));
        ...
        if ($this->config->removeuser == AUTH_REMOVEUSER_FULLDELETE) {
            ...
            if (! empty($remove_users)) {
                ...
                foreach ($remove_users as $user) {
                    if (delete_user($user)) {
                        echo "\t";
                        print_string('auth_dbdeleteuser', 'auth_db', array(
                            'name' => $user->username,
                            'id' => $user->id
                        ));
                        echo "\n";
            ...
            if ($this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
                ...
                if (! empty($remove_users)) {
                    ...
                    foreach ($remove_users as $user) {
                        ...
                        echo "\t";
                        print_string('auth_dbsuspenduser', 'auth_db', array(
                            'name' => $user->username,
                            'id' => $user->id
                        ));
                        echo "\n";
        ...
        if (! empty($this->config->removeuser) and $this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
            ...
            $revive_users = $DB->get_records_sql($sql, array(
                $this->authtype
            ));
            ...
            if (! empty($revive_users)) {
                ...
                foreach ($revive_users as $user) {
                    ...
                    user_update_user($updateuser, false);
                    echo "\t";
                    print_string('auth_dbreviveduser', 'auth_db', array(
                        'name' => $user->username,
                        'id' => $user->id
                    ));
                    echo "\n";
        ...
        if ($do_updates and ! empty($updatekeys)) { // run updates only if relevant
            $users = $DB->get_records_sql('SELECT u.username, u.id
                                             FROM {user} u
                                            WHERE u.deleted = 0 AND u.auth = ? AND u.mnethostid = ?', array(
                $this->authtype,
            ...
            if (! empty($users)) {
                ...
                foreach ($users as $user) {
                    echo "\t";
                    print_string('auth_dbupdatinguser', 'auth_db', array(
                        'name' => $user->username,
                        'id' => $user->id
                    ));
        ...
        if (! empty($add_users)) {
            ...
            foreach ($add_users as $user) {
                ...
                echo "\t";
                print_string('auth_dbinsertuser', 'auth_db', array(
                    'name' => $user->username,
                    'id' => $id
                ));
                echo "\n";
                $euser = $DB->get_record('user', array(
                    'id' => $id
                ));
     ...
     *            true to update the local record with the external LDAP values.
     * @param bool $triggerevent
     *            set false if user_updated event should not be triggered.
    ...
    function ldap_bulk_insert($username)
    {
        ...
        $DB->insert_record_raw('tmp_extuser', array(
            'username' => $username,
            'mnethostid' => $CFG->mnet_localhost_id
        ), false, true);
        echo '.';
    ...
    function user_activate($username)
    {
        ...
        switch ($this->config->user_type) {
            ...
            case 'rfc2307bis':
                ...
                // need to remove it.
                $sr = ldap_read($ldapconnection, $userdn, '(objectClass=*)', array(
                    'userPassword'
                ));
            ...
            case 'ad':
                ...
                $info = ldap_get_entries($ldapconnection, $sr);
                $info[0] = array_change_key_case($info[0], CASE_LOWER);
                $newinfo['userAccountControl'] = $info[0]['useraccountcontrol'][0] & (~ AUTH_AD_ACCOUNTDISABLE);
                break;
            default:
    ...
    function iscreator($username)
    {
        ...
        for ($x = 0; $x < $numberOfServers; $x ++) {
            ...
            $user_attribute = json_decode($this->config->user_attribute);
            $search_sub = json_decode($this->config->search_sub);
            $creators = json_decode($this->config->creators);
            $memberattribute = json_decode($this->config->memberattribute);
            $memberattribute_isdn = json_decode($this->config->memberattribute_isdn);
            
            $extusername = core_text::convert($username, 'utf-8', $ldapencoding[$x]);
            
            $ldapconnection = $this->ldap_connect($hosts[$x], $verstions[$x], $types[$x], $bindsDn[$x], $bindsPw[$x], $optsDeref[$x], $startsTls[$x]);
            
            if (empty($creators[$x]) or empty($memberattribute[$x])) {
                return null;
            }
            ...
            $group_dns = explode(';', $creators[$x]);
            
            $creator = ldap_isgroupmember($ldapconnection, $userid, $group_dns, $memberattribute[$x]);
    ...
    function user_update($olduser, $newuser)
    {
        ...
        if ($user_info_result) {
            ...
            if (empty($user_entry)) {
                ...
                error_log($this->errorlogtag . get_string('updateusernotfound', 'auth_ldap', array(
                    'userdn' => $user_dn,
                    'attribs' => $attribs
                )));
            ...
            foreach ($attrmap as $key => $ldapkeys) {
                ...
                if (isset($olduser->$key) and isset($newuser->$key) and ($olduser->$key !== $newuser->$key)) {
                    $profilefield = $key;
                } else 
                    if (isset($olduser->$customprofilefield) && isset($newuser->$customprofilefield) && $olduser->$customprofilefield !== $newuser->$customprofilefield) {
                ...
                if (! empty($profilefield) && ! empty($this->config->{'field_updateremote_' . $key})) {
                    ...
                    foreach ($ldapkeys as $ldapkey) {
                        ...
                        if (! $ambiguous) {
                            ...
                            if ($nuvalue !== $ldapvalue) {
                                ...
                                if (@ldap_modify($ldapconnection, $user_dn, array(
                                    $ldapkey => $nuvalue
                                ))) {
                                    $changed = true;
                                    continue;
                                    ...
                                    $success = false;
                                    error_log($this->errorlogtag . get_string('updateremfail', 'auth_ldap', array(
                                        'errno' => ldap_errno($ldapconnection),
                    ...
                    if ($ambiguous and ! $changed) {
                        $success = false;
                        error_log($this->errorlogtag . get_string('updateremfailamb', 'auth_ldap', array(
                            'key' => $key,
                            'ouvalue' => $ouvalue,
                            'nuvalue' => $nuvalue
                        )));
     ...
     *            Plaintext password (not crypted/md5'ed)
     * @return boolean result
     *        
     */
    function user_update_password($user, $newpassword)
    {
        ...
        switch ($this->config->user_type) {
            case 'edir':
                // Change password
                $result = ldap_modify($ldapconnection, $user_dn, array(
                    'userPassword' => $extpassword
                ));
                if (! $result) {
                    error_log($this->errorlogtag . get_string('updatepasserror', 'auth_ldap', array(
                        'errno' => ldap_errno($ldapconnection),
                        'errstring' => ldap_err2str(ldap_errno($ldapconnection))
                    )));
                }
                ...
                if ($sr) {
                    ...
                    if (! empty($info[$this->config->expireattr][0])) {
                        ...
                        if (! $result) {
                            error_log($this->errorlogtag . get_string('updatepasserrorexpiregrace', 'auth_ldap', array(
                                'errno' => ldap_errno($ldapconnection),
                                'errstring' => ldap_err2str(ldap_errno($ldapconnection))
                            )));
                        }
                    ...
                    error_log($this->errorlogtag . get_string('updatepasserrorexpire', 'auth_ldap', array(
                        'errno' => ldap_errno($ldapconnection),
                        'errstring' => ldap_err2str(ldap_errno($ldapconnection))
                    )));
                }
     ...
     *            Only needed for Active Directory.
     * @param string $user_dn
     *            User distinguished name for the user we are checking password expiration (only needed for Active Directory).
     * @return timestamp
     */
    ...
    function ldap_attributes()
    {
        ...
        for ($x = 0; $x < $numberOfServers; $x ++) {
            ...
            $user_attribute = json_decode($this->config->user_attribute);
            $search_sub = json_decode($this->config->search_sub);
            $suspended_attribute = json_decode($this->config->suspended_attribute);
            ...
            $moodleattributes['username'] = core_text::strtolower(trim($user_attribute[$x]));
            $moodleattributes['suspended'] = core_text::strtolower(trim($suspended_attribute[$x]));
     ...
     *
     * @param $filter An
     *            LDAP search filter to select desired users
    ...
    {
        return ! empty($stdchangepassword) or ! empty($changepasswordurl);
    }
...
($_SERVER['REQUEST_METHOD'] === 'POST' && (get_local_referer() != strip_querystring(qualified_me())))) && 
...
address_in_subnet(getremoteaddr(), $this->config->ntlmsso_subnet)) {
     ...
     *
     * @param $user object
     *            user object (without system magic quotes)
     */
    ...
    function config_form($config, $err, $user_fields)
    {
        ...
        include ($CFG->dirroot . '/auth/ldap_multi/config.html');
    }
    ...
    function process_config($config)
    {
        ...
        // Save settings
        set_config('host_url', $this->prepareValue($config->host_url), $this->pluginconfig);
        set_config('start_tls', $this->prepareValue($config->start_tls), $this->pluginconfig);
        set_config('ldapencoding', $this->prepareValue($config->ldapencoding), $this->pluginconfig);
        set_config('pagesize', $this->prepareValue($config->pagesize), $this->pluginconfig);
        set_config('contexts', $this->prepareValue($config->contexts), $this->pluginconfig);
        set_config('user_type', $this->prepareValue($config->user_type), $this->pluginconfig);
        set_config('user_attribute', $this->prepareValue($config->user_attribute), $this->pluginconfig);
        set_config('suspended_attribute', $this->prepareValue($config->suspended_attribute), $this->pluginconfig);
        set_config('sync_suspended', $this->prepareValue($config->sync_suspended), $this->pluginconfig);
        set_config('search_sub', $this->prepareValue($config->search_sub), $this->pluginconfig);
        set_config('opt_deref', $this->prepareValue($config->opt_deref), $this->pluginconfig);
        set_config('preventpassindb', $this->prepareValue($config->preventpassindb), $this->pluginconfig);
        set_config('bind_dn', $this->prepareValue($config->bind_dn), $this->pluginconfig);
        set_config('bind_pw', $this->prepareValue($config->bind_pw, 'bind_pw'), $this->pluginconfig);
        set_config('ldap_version', $this->prepareValue($config->ldap_version), $this->pluginconfig);
        set_config('objectclass', $this->prepareValue($config->objectclass), $this->pluginconfig);
        set_config('memberattribute', $this->prepareValue($config->memberattribute), $this->pluginconfig);
        set_config('memberattribute_isdn', $this->prepareValue($config->memberattribute_isdn), $this->pluginconfig);
        set_config('creators', $this->prepareValue($config->creators), $this->pluginconfig);
        set_config('create_context', $this->prepareValue($config->create_context), $this->pluginconfig);
        set_config('expiration', $this->prepareValue($config->expiration), $this->pluginconfig);
        set_config('expiration_warning', $this->prepareValue($config->expiration_warning), $this->pluginconfig);
        set_config('expireattr', $this->prepareValue($config->expireattr), $this->pluginconfig);
        set_config('gracelogins', $this->prepareValue($config->gracelogins), $this->pluginconfig);
        set_config('graceattr', $this->prepareValue($config->graceattr), $this->pluginconfig);
        set_config('auth_user_create', $this->prepareValue($config->auth_user_create), $this->pluginconfig);
        set_config('forcechangepassword', $this->prepareValue($config->forcechangepassword), $this->pluginconfig);
        set_config('stdchangepassword', $this->prepareValue($config->stdchangepassword), $this->pluginconfig);
        set_config('passtype', $this->prepareValue($config->passtype), $this->pluginconfig);
        set_config('changepasswordurl', $this->prepareValue($config->changepasswordurl), $this->pluginconfig);
        set_config('removeuser', $this->prepareValue($config->removeuser), $this->pluginconfig);
        set_config('ntlmsso_enabled', $this->prepareValue($config->ntlmsso_enabled), $this->pluginconfig);
        set_config('ntlmsso_subnet', $this->prepareValue($config->ntlmsso_subnet), $this->pluginconfig);
        set_config('ntlmsso_ie_fastpath', $this->prepareValue($config->ntlmsso_ie_fastpath), $this->pluginconfig);
        set_config('ntlmsso_type', $this->prepareValue($config->ntlmsso_type), 'auth/ldap');
        set_config('ntlmsso_remoteuserformat', $this->prepareValue($config->ntlmsso_remoteuserformat), 'auth/ldap');
        
        return true;
    }

    private function prepareValue($value, $field = null)
    {
        if (! is_array($value)) {
            $value = trim($value);
            return json_encode([
     ...
     *
     * @param string $pwdlastset
     *            The time last time we changed the password.
     * @param resource $lcapconn
     *            The open LDAP connection.
     * @param string $user_dn
     *            The distinguished name of the user we are checking.
     *            
    ...
    function ldap_get_ad_pwdexpire($pwdlastset, $ldapconn, $user_dn)
    {
        ...
        $sr = ldap_read($ldapconn, $user_dn, '(objectClass=*)', array(
            'userAccountControl'
        ));
        if (! $sr) {
        ...
        $sr = ldap_read($ldapconn, ROOTDSE, '(objectClass=*)', array(
        ...
        $sr = ldap_read($ldapconn, $domaindn, '(objectClass=*)', array(
            'maxPwdAge'
        ));
        $entry = ldap_get_entries_moodle($ldapconn, $sr);
        $info = array_change_key_case($entry[0], CASE_LOWER);
        $maxpwdage = $info['maxpwdage'][0];
        if ($sr = ldap_read($ldapconn, $user_dn, '(objectClass=*)', array(
            'msDS-ResultantPSO'
        ))) {
            if ($entry = ldap_get_entries_moodle($ldapconn, $sr)) {
                ...
                if (! empty($userpso)) {
                    $sr = ldap_read($ldapconn, $userpso, '(objectClass=*)', array(
     ...
     */
    function ldap_connect($host_url, $ldap_version, $user_type, $bind_dn, $bind_pw, $opt_deref, $start_tls)
    {
        ...
        if ($ldapconnection = ldap_connect_moodle($host_url, $ldap_version, $user_type, $bind_dn, $bind_pw, $opt_deref, $debuginfo, $start_tls)) {
            $this->ldapconns = 1;
     ...
     *
     * @param
     *            force boolean Forces closing the real connection to the LDAP server, ignoring any
     ...
     *            a valid LDAP connection
     * @param string $extusername
     *            the username to search (in external LDAP encoding, no db slashes)
     * @return mixed the user dn (external LDAP encoding) or false
     */
    function ldap_find_userdn($ldapconnection, $extusername, $context, $create_context, $objectclass, $user_attribute, $search_sub)
    {
        $ldap_contexts = explode(';', $context);
        
        if (! empty($create_context)) {
            array_push($ldap_contexts, $create_context);
        }
        
        return ldap_find_userdn($ldapconnection, $extusername, $ldap_contexts, $objectclass, $user_attribute, $search_sub);
    }
     ...
     * @param array $err
     *            array of error messages (passed by reference)
     */
     ...
     * So we let the admin configure the format using two
    ...
    protected function get_ntlm_remote_user($remoteuser)
    {
        ...
        $formatregex = preg_replace(array(
            '#%domain%#',
            '#%username%#'
        ), array(
            '(' . AUTH_NTLM_VALID_DOMAINNAME . ')',
     ...
     * This function is intended to be used after calling
